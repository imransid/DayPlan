...
function defaultApiUrl(): string {
  if (Platform.OS === "android")
    return "https://new-plan-292b.onrender.com/api";
...
function readNativeConfig(): Record<string, unknown> {
  try {
    ...
    const NativeModule =
      require("react-native-config/codegen/NativeConfigModule").default;
...
const envApiUrl =
  typeof envApiUrlRaw === "string" && envApiUrlRaw.length > 0
    ? envApiUrlRaw
    : undefined;
